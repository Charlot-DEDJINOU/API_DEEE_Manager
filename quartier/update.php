...
if( !empty($data->id) && !empty($data->nom_quartier) ){

    $quartier->nom_quartier = htmlentities($data->nom_quartier) ;
    $quartier->id = htmlentities($data->id) ;
    
    $stmt = $quartier->read_one_by_id() ;
    ...
    if($num == 1){
        ...
        http_response_code(201) ;
        echo json_encode(array(
                "message"=>"Cette quartier n'exite pas"
            )) ;
    }
